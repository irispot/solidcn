...
const fixtureJsx: Plugin = {
  name: 'unchanged-checkbox-group-hydration-fixture-jsx',
  enforce: 'pre',
...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: [
      ...
      { find: /^@mui\/internal-test-utils$/, replacement: tooling('checkbox-group-hydration-mui-entry.ts') },
  ...
  test: {
    name: 'unchanged-checkbox-group-native-solid-ssr-hydration',
    environment: 'jsdom',
    globals: true,
    include: ['base-ui/packages/react/src/checkbox-group/CheckboxGroup.test.tsx'],
    testNamePattern: 'during SSR|labels the group rather|points parent aria-controls',
    setupFiles: [tooling('checkbox-group-hydration-setup.ts')],
    maxWorkers: 1,
