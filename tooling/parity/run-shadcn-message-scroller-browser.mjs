...
const originalTest = 'shadcn-ui/packages/react/src/message-scroller/message-scroller.browser.test.tsx';
const nativeSource = resolve(root, 'shadcn-ui/packages/solid/src/internal/message-scroller.tsx');
const reportPath = resolve(root, 'artifacts/unchanged-shadcn-message-scroller-browser-comparison.json');
const temporary = mkdtempSync(resolve(tmpdir(), 'solid-cn-message-scroller-browser-'));
...
function run(mode, config) {
  ...
  if (report.testResults?.length !== 1 || relative(root, suite?.name ?? '') !== originalTest)
    errors.push(`${mode} did not report the exact unchanged MessageScroller browser test.`);
  return {
    ...
    routedToNative: output.includes(`Solid message-scroller browser route: ${nativeSource}`),
...
try {
  ...
  const react = run('react', 'tooling/parity/reference-shadcn-message-scroller-browser.config.ts');
  const solid = run('solid', 'tooling/parity/dual-shadcn-message-scroller-browser-solid.config.ts');
  ...
  for (const [mode, result] of [['React', react], ['Solid', solid]]) {
    if (result.exitCode !== 0 || result.counts.total !== 17 || result.counts.passed !== 17 ||
        result.counts.failed !== 0 || result.counts.skipped !== 0 ||
        result.assertions.length !== 17 || result.unhandledErrors.length || result.outputHasUnhandledErrors)
      errors.push(`${mode} did not pass all 17 original MessageScroller browser assertions.`);
  }
  if (!solid.routedToNative || solid.nativeCaseEvidenceCount !== 17)
    errors.push('The unchanged browser test did not execute native Solid MessageScroller in all 17 cases.');
  ...
  console.log(JSON.stringify({ success: evidence.success, react: react.counts, solid: solid.counts,
    nativeRoute: solid.routedToNative && solid.nativeCaseEvidenceCount === 17,
