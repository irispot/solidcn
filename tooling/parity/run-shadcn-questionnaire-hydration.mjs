...
const reportPath = resolve(root, 'artifacts/unchanged-shadcn-questionnaire-hydration-comparison.json');
const temporary = mkdtempSync(resolve(tmpdir(), 'solid-cn-questionnaire-hydration-'));
const errors = [];
...
function run(mode, config) {
  ...
  if (report.testResults?.length !== 1 || relative(root, suite.name) !== originalTest)
    errors.push(`${mode} did not load the exact unchanged Questionnaire test.`);
  return {
...
try {
  ...
  const solid = run('solid', 'tooling/parity/dual-shadcn-questionnaire-hydration-solid.config.ts');
  ...
  for (const [mode, result] of [['React', react], ['Solid', solid]]) {
    if (result.exitCode !== 0 || result.counts.passed !== 10 || result.counts.failed !== 0 ||
        result.counts.skipped !== 9 || result.counts.total !== 19)
      errors.push(`${mode} did not pass the 10 original hydration assertions.`);
  }
  if (!solid.routedToNative || solid.nativeCaseEvidenceCount < 10 || solid.hydrationEvidenceCount !== 10)
    errors.push('The unchanged test did not hydrate the native Solid Questionnaire in all 10 cases.');
  ...
  const evidence = {
    ...
    originalTest,
    selectedGroup: 'Questionnaire hydration',
    react,
    solid,
  ...
  console.log(JSON.stringify({
    ...
    react: react.counts,
    solid: solid.counts,
    nativeHydration: solid.routedToNative && solid.nativeCaseEvidenceCount >= 10 && solid.hydrationEvidenceCount === 10,
