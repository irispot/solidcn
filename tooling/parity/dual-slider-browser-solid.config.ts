...
import clientConfig from './dual-slider-client-solid.config.ts';

const config = mergeConfig(clientConfig, {
  test: {
    name: 'unchanged-slider-native-solid-chromium',
    browser: {
      enabled: true,
      provider: playwright(),
      headless: true,
...
config.test!.setupFiles = [resolve(import.meta.dirname, 'slider-browser-fixture-setup.ts')];
