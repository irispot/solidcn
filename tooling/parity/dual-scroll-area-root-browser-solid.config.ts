...
import clientConfig from './dual-scroll-area-root-client-solid.config.ts';

const config = mergeConfig(clientConfig, {
  test: {
    name: 'unchanged-scroll-area-root-native-solid-chromium',
    browser: {
      enabled: true,
      provider: playwright(),
      headless: true,
...
config.resolve!.alias = [
  { find: /^#test-utils$/, replacement: resolve(import.meta.dirname, 'scroll-area-browser-fixture-entry.ts') },
