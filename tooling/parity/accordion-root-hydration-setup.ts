...
const service = await server.ssrLoadModule('/tooling/parity/accordion-root-hydration-ssr-service.ts');
(globalThis as any).__solidCnAccordionRootSsr = service.renderAccordionRootToString;
afterAll(async () => {
  delete (globalThis as any).__solidCnAccordionRootSsr;
  await server.close();
});
