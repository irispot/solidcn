...
const service = await server.ssrLoadModule('/tooling/parity/tabs-indicator-hydration-ssr-service.ts');
(globalThis as any).__solidCnTabsIndicatorSsr = service.renderTabsIndicatorToString;
afterAll(async () => {
  delete (globalThis as any).__solidCnTabsIndicatorSsr;
  await server.close();
});
